 ...
 */
const TripleFeatureShowcase = ({ data, items, containerClassName }: TripleFeatureShowcaseProps): JSX.Element => {
    const firstItem = items[0]
    const secondItem = items[1]
    const thirdItem = items[2]
    if (!(firstItem.title && secondItem.title && thirdItem.title)) return (<p>The slice requires three items</p>)

  return (
    <Section>
        <div className={`container ${containerClassName || ""}`}>
            <Heading title={data.heading} text={data.text} />
            <div className="relative">
                <div className="relative z-1 flex items-center h-[38.75rem] mb-5 p-8 border border-n-1/10 rounded-3xl overflow-hidden lg:h-[38.75rem] lg:p-20 xl:h-[45.75rem]">
                    <div className="absolute top-0 left-0 w-full h-full pointer-events-none md:w-3/5 xl:w-auto">
                        {isFilled.image(firstItem.image) 
                            ? <PrismicNextImage 
                            field={firstItem.image}
                    ...
                    <div className="relative z-1 max-w-[17rem] ml-auto">
                        <h4 className="h4 mb-4">{firstItem.title}</h4>
                        {isFilled.richText(firstItem.text) && <PrismicRichText 
                            field={firstItem.text}
                            components={{
                                paragraph: ({children}) => 
                        ...
                        <ul className="body-2">
                            {[
                                firstItem.feature_tagline_1,
                                firstItem.feature_tagline_2,
                                firstItem.feature_tagline_3,
                            ].map((item, index) => (<>{ isFilled.keyText(item) &&
                                 <li
                ...
                <div className="relative z-1 grid gap-5 lg:grid-cols-2">
                    ...
                    <div className="relative min-h-[38.75rem] border border-n-1/10 rounded-3xl overflow-hidden">
                        ...
                        <div className="absolute inset-0 flex flex-col justify-end p-8 bg-gradient-to-b from-n-8/0 to-n-8/90 lg:p-15">
                            <h4 className="h4 mb-4">{secondItem.title}</h4>
                            {isFilled.richText(secondItem.text) && <PrismicRichText 
                                field={secondItem.text}
                                components={{
                    ...
                    {isFilled.keyText(thirdItem?.title) &&
                    <div className="p-4 bg-n-7 rounded-3xl overflow-hidden lg:min-h-[45.75rem]">
                        <div className="py-12 px-4 xl:px-8">
                            <h4 className="h4 mb-4">{thirdItem.title}</h4>
                            {isFilled.richText(thirdItem.text) && <PrismicRichText 
                                field={thirdItem.text}
                                components={{
                                    paragraph: ({children}) =>  <p className="body-2 mb-[2.25rem] text-n-3">{children}</p>
                                }}
                            />}
...
export default TripleFeatureShowcase;
